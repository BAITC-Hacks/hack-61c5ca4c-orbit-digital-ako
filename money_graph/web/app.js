...
function renderCard() {
  ...
  detail(kv,"Приоритет",Number(n.priority_score).toFixed(3)); detail(kv,"Оценка правила роли",Number(n.role_score).toFixed(2));
...
function renderGraph(remember=true) {
  ...
  $("graph-heading").textContent=state.cluster===null?"Окружение · …"+state.focus.slice(-7):"Кластер "+state.cluster;
