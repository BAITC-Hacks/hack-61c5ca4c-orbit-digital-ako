...
DISCLAIMER = (
    ...
    "отсутствие связи не доказывает отсутствие перевода. LLM не подключена; внешние AI-запросы не выполняются."
)
...
def _retrieve(context, question):
    ...
    preamble = "Локальный поиск; LLM не подключена. Ответ НЕ сгенерирован LLM."
    if "rag" in terms or "раг" in terms:
        preamble += " Поиск оснований готов для RAG; генеративная модель не настроена."
    if sources:
