...
class GraphTools:
    def __init__(self, data: Path, out: Path):
        self.nodes = {r["gid"]: r for r in records(out / "nodes_roles.csv", ("gid",))}
        ...
        frame = pd.read_parquet(data / "edges.parquet")
        ...
        trace_path = out / "taint_edges.csv"
        ...
        self.requests = records(out / "requests.csv", ("gid",))
        self.summary = json.loads((out / "summary.json").read_text(encoding="utf-8"))
    ...
    def get_node_details(self, gids):
        ...
        for gid in gids:
            node = self.nodes[gid]
            role_label = (f"Обрезан на {self.summary.get('max_depth', 4)}-м хопе"
    ...
    def find_common_recipients(self, gids):
        ...
        for gid, links in common[:10]:
            ...
            reason = f"Получил {money(amount)} от {len(links)} из {len(gids)} выбранных счетов."
...
async def ask_openai(graph: GraphTools, gids: list[str], question: str, client=None):
    ...
    messages = [{"role": "user", "content": json.dumps({"question": question, "selected_gids": gids,
                 "period_start": graph.summary.get("period_start"), "period_end": graph.summary.get("period_end"),
