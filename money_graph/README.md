...
Повторно используется `mg/assistant.py` из коммита `ab08d0d`: объяснение узлов, общие получатели, направленный путь и запрос недостающих данных. Адаптер `api/assistant.py` передаёт ему версию расчёта выбранного проекта; идентификаторы остаются строками, валюта берётся из профиля. Ответ содержит проверяемые ссылки на узлы и связи и кнопку перехода на граф.
...
В прежней версии `main` также появилась другая, хронологическая модель движения денег и отдельная система пользователей/документов. Они не включены в рабочую реализацию: текущий числовой эталон явно задан вашим ТЗ. История обеих веток объединена merge-коммитом с приоритетом текущего MVP; предыдущая реализация сохранена в истории (`ab08d0d`). Функционально интегрирован ассистент. Для объединения моделей следует добавить отдельный режим, собственные эталонные тесты и сравнение результатов.
...
4. При необходимости включить внешний ассистент: настроить ключ, выбрать допустимые для передачи данные и проверить ответы на примерах аналитика. Для интеграции дополнительных модулей из `ab08d0d` отдельно согласовать режим расчёта и перенести разграничение доступа на все новые API проекта.
