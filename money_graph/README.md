...
### Помощник по выбранным счетам

После входа в разделе «Расследование» добавьте в панель «Разобрать узлы и связи» от одного до пяти узлов через карточку или кнопку «Выбранный узел». Четыре действия работают локально и бесплатно: объяснение узлов, общие получатели, направленный путь и запросы недостающих данных. Для пути нужны ровно два счёта; направление задаётся порядком добавления. Общий получатель означает поступления хотя бы от двух выбранных счетов, а не обязательно от всех. Ссылки в ответе открывают узлы, подтверждающие связи подсвечиваются на карте. Помощник проверяет владельца кейса; при выходе его корзина, вопрос и ответ очищаются.

Для вопросов своими словами установите необязательные зависимости и создайте файл настроек (команды из `money_graph/`):
...
В `.env` укажите `OPENAI_API_KEY` и при необходимости `OPENAI_MODEL` (по умолчанию `gpt-4.1-mini`), затем перезапустите сервер. Ключ не вводится в браузере и исключён из Git. Существующие переменные окружения имеют приоритет над `.env`. Без ключа или OpenAI SDK все локальные функции остаются доступны.

Серверу нужен исходящий доступ к `api.openai.com:443`. Надпись «Ключ настроен» означает наличие настройки; доступ проверяется при отправке вопроса. Ошибки соединения, ключа, квоты и лимита показываются отдельно. Ошибка ключа OpenAI не завершает вход в аккаунт приложения. Поиск во вкладке «Документы и поиск» выполняется локально и не использует API-ключ.

Только кнопка «Спросить ИИ» отправляет вопрос, выбранные синтетические gid, параметры выборки и ограниченные результаты серверных инструментов в OpenAI. Весь датасет не загружается. Используется Responses API с четырьмя инструментами чтения графа, максимум четырьмя запросами модели, шестью вызовами инструментов, 2 000 выходных токенов на запрос и общим тайм-аутом 60 секунд. Повторные попытки SDK выключены; каждый новый вопрос может расходовать баланс API. Одновременно выполняется один вопрос. Если пользователь закрыл запрос в браузере, уже отправленный платный запрос может завершиться на сервере.

Результат ИИ проходит проверку формата и ссылок: можно сослаться только на узлы и связи, полученные инструментами в этом вопросе. Это не проверка достоверности каждой фразы; выводы необходимо сверять с фактами. ИИ не изменяет роли, данные или результаты расчёта. У помощника нет инструментов выполнения произвольного кода или доступа к внешним источникам.

Запросы используют `store=False`; это не означает полного отсутствия хранения у провайдера. Условия хранения следует учитывать при использовании реальных данных. Справка: [function calling](https://developers.openai.com/api/docs/guides/function-calling), [Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs), [обработка данных API](https://developers.openai.com/api/docs/guides/your-data).
...
| `taint_edges.csv` | суммы по связям, поддержанные хронологической моделью: `src,dst,sum_kzt,n_tx,tainted_kzt,taint_share` |
...
flowchart LR
  ...
  A --> C[Меченые деньги<br>даты, доступный остаток,<br>пропорциональное смешивание]
...
**Хронологическая модель.** Транзакции обрабатываются по календарным дням. В начале дня исходящие переводы расходуют только остаток от предыдущих дней, затем зачисляются поступления текущего дня. При смешивании меченых и немеченых денег доля метки распределяется пропорционально. На все исходящие за день нельзя списать больше накопленной меченой суммы. Недостаток видимого остатка считается неизвестным немеченым финансированием; будущие поступления его не покрывают задним числом.

Все исходящие seed помечаются по определению модели, включая новые вливания неизвестного происхождения. Входящая доля метки у seed при этом считается по фактическим входам и не обязана равняться единице. Порядок операций внутри одного дня неизвестен, поэтому такое продолжение цепочки не учитывается как подтверждённое моделью. Начальные остатки и внешние поступления неизвестны. Эти допущения сохранены в `summary.json` и видны в интерфейсе.

`tainted_flow_kzt` — модельный объём переводов, а не сумма уникальных средств: одна сумма может учитываться на нескольких последовательных рёбрах и в хронологическом цикле. `taint_edges.csv` показывает результат по каждой связи. Прежний параметр `taint_iterations` сохранён для совместимости конфигурации и больше не используется.

**Сценарий исключения.** Для каждого узла удаляем его операции и заново выполняем тот же расчёт. Это изменение внутри модели, а не прогноз реальной блокировки или поведения участников. При исключении немеченого донора пропорция смешивания иногда увеличивает меченый поток: отрицательный `block_impact` сохраняется и объясняется явно, а вклад в приоритет учитывает только положительный эффект. Сравнение топ-N со случайными N находится в `resilience.csv`; оно показывает поведение выбранной модели и не доказывает точность ролей. Актуальные суммы и результаты всегда берутся из свежего `out/`.

Компонент эффекта исключения уже участвует в ранжировании, поэтому сравнение с произвольными узлами не является независимой оценкой точности. [Определения полей и ограничения модели](docs/MODEL-LIMITS.md).

## Роли: правила и пороги

Правила проверяются по порядку, срабатывает первое. Все пороги в `config.json`. `role_score` — сила срабатывания правила, а не измеренная вероятность его правильности. Для правил с порогом оценка растёт от 0,5 до 1,0 при двукратном превышении.
...
| 5 | `transit` | не seed, есть вход и выход, и либо передал 70–130% полученного, либо ≥70% исходящей суммы покрыто поступлениями предыдущих 1–2 дней по FIFO | 104 |
| 6 | `terminal` | нет исходящих на **полностью выгруженном** хопе 0–3, и получил ≥100 тыс или от ≥2 плательщиков | 358 |
| 7 | `peripheral` | всё остальное | 1 193 |
...
`fast_share` — доля исходящей суммы, покрытая неиспользованными поступлениями предыдущих дней в пределах `fast_lag_days`. Сопоставление идёт по FIFO (сначала расходуются самые старые поступления); одна входящая сумма не используется повторно. Это отдельное допущение для временного признака, отличное от пропорционального смешивания в модели меток. Поступления того же дня учитываются только как неоднозначность. В CSV дополнительно сохранены `matched_out_kzt`, `unmatched_out_kzt` и `same_day_overlap_kzt`.
...
В `nodes_roles.csv` сохранены шесть нормированных вкладов `priority_*`; их сумма равна итоговому баллу с точностью округления. В карточке узла виден вклад каждого фактора. Это позволяет объяснить его место в очереди проверки.
...
Открыть `viewer.html` → «Поиск gid» → карточка узла показывает роль, evidence, все метрики правила и крупнейшие входящие/исходящие. Пример: `100000003684369100` — seed, получает от 24 и переводит 62 получателям, по посредничеству в топ-1% → `coordinator`. Вклад факторов и эффект исключения берутся из текущего расчёта.
...
- Меченые деньги — векторные дневные пакеты; для больших объёмов нужны последовательная обработка транзакций по датам и хранение остатков. Производительность необходимо измерять на целевом объёме.
...
mg/assistant.py     инструменты чтения графа и необязательный OpenAI-помощник
...
Для проверки состояний интерфейса нужен Node.js 18+; установка npm-пакетов не требуется. Она проверяет доступность кнопок при первой загрузке, повторной попытке и смене кейса.

Проверки выходных файлов выполняют полный свежий расчёт исходного датасета во временную папку, а не читают сохранённый `out/`. Синтетические сценарии проверяют порядок дат, ограничение суммы, повторное расходование, циклы, неоднозначность одного дня и исключение узлов. Тесты помощника проверяют локальные действия, формат и происхождение ссылок, лимиты запросов, ошибки и HTTP-интеграцию. OpenAI заменён тестовым клиентом: эти тесты не отправляют данные провайдеру и не расходуют API-баланс. Для проверки живого ИИ нужен ключ и отдельный запрос из интерфейса.
