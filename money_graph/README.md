...
Нужен **Python 3.10+** (используется синтаксис `X | None` в аннотациях типов). Перейдите в папку `money_graph/` и установите зависимости:
...
Проверено на чистом venv (Python 3.10.9, установка только из `requirements.txt`) — пайплайн и валидатор отрабатывают без ручных шагов.

## Интерфейс аналитика

![Панель анализа: список приоритетов, граф и карточка узла](docs/ui-overview.png)
...
Исходники экрана находятся в `ui/`; `mg/viewer.py` встраивает их в один автономный HTML при запуске пайплайна. Все gid передаются в браузер как строки, чтобы сохранить точность идентификаторов.
...
`ANTHROPIC_API_KEY=... python run.py --llm` переписывает гипотезы 15 крупнейших кластеров человеческим языком. LLM не участвует в назначении ролей и приоритета: они остаются объяснимыми правилами. Без ключа или при ошибке API пайплайн оставляет шаблонные гипотезы и не падает.
...
mg/llm.py           опциональная LLM-переформулировка гипотез
